...
class CheckTexto:
    def __init__(self, texto):
        self.texto = texto
        # quitamos todos los simbolos en el texto
        self.palabras = "".join(e for e in texto if e.isalnum() or e.isspace()).split()
        # obtenemos la cantidad total de palabras presentes en el texto ya depurado
        self.num_palabras = len(self.palabras)
        # obtenemos la cantidad de letras presentes
        self.longitud_total = sum(len(palabra) for palabra in self.palabras)
        # obtenemos la cantidad de oraciones
        self.num_oraciones = texto.count('.')
        # obtenemos la palabra mas larga presente
    ...
    def __str__(self):
        return f"Cantidad de letras: {self.longitud_total} \n" \
...
"""
<Salida>
Número total de palabras: 141
Longitud media de las palabras: 5.12
Número de oraciones del texto: 10
Palabra más larga: 'programadores', con: 13 letras
"""
